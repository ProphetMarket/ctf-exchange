...
abstract contract Trading is IFees, ITrading, IHashing, IRegistry, ISignatures, INonceManager, IAssetOperations {
    ...
    function _matchOrders(
        ...
        // Calculate fee on the signed taking amount BEFORE delta accounting inflates it.
        // The user consented to feeRateBps against their signed price — surplus is a
        // price improvement that should not increase the fee (M-03).
        uint256 fee = CalculatorHelper.calculateFee(
        ...
        uint256 makerAssetBefore;
        {
            // Snapshot balances BEFORE any transfers to use delta-based accounting.
            // This prevents pre-existing or donated tokens from inflating payouts (H-04, H-06).
            uint256 takerAssetBefore = _getBalance(takerAssetId);
            makerAssetBefore = _getBalance(makerAssetId);
            ...
            // Use delta-based accounting: only count tokens generated by the fills
            taking = _updateTakingWithDelta(taking, takerAssetId, takerAssetBefore);
        }
        ...
        // Refund only tokens that were pulled from the taker (delta), not pre-existing balance
        {
            uint256 refund = _getBalance(makerAssetId) - makerAssetBefore;
    ...
    /// @notice Computes the actual taking amount using delta-based accounting.
    /// Only counts tokens generated by the fills (current - snapshot), ignoring
    /// any pre-existing balance that could be exploited via donation attacks.
    function _updateTakingWithDelta(uint256 minimumAmount, uint256 tokenId, uint256 balanceBefore)
        internal
        returns (uint256)
    {
        uint256 actualAmount = _getBalance(tokenId) - balanceBefore;
